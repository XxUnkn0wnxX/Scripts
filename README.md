...
- `brew-custom-compare.zsh`  
  Scans a tap’s formulas, fetches their stable versions via `brew info --json=v2`, and compares them with the official Homebrew JSON API so you can see which of your patched formulae are ahead, behind, or missing upstream equivalents. After the API check, it automatically inspects every other tapped repository (excluding `homebrew/core`) and reports only the overlaps—handy when you’ve forked a formula that also lives in another tap.
