...
- Optional `--gateway`, `--metric`, `--no-comments`, `--force`, and `--verbose` flags
...
```bash
python3 vpnroute.py sites.txt --output viscosity_routes.txt --force
```
...
Failed domains are emitted as comments unless `--no-comments` is used:

```conf
# FAILED: example.invalid - no IPv4 records found
```
...
If the destination already exists, pass `--force` to replace it.
