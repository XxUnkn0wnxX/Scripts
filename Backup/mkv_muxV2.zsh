...
boost_audio_volume() {
  ...
  volume_change_array=(${(s:,:)volume_changes})

  # Extract original audio track
  extracted_file="extracted_audio.${codec_extension}"
  ...
  # Loop over each volume change and create a boosted audio file
  temp_files=()
  for volume_change in "${volume_change_array[@]}"; do
    boosted_file="./${volume_change}.${codec_extension}"
    echo "Boosting volume by ${volume_change}..."
    ffmpeg -y -i "$extracted_file" -filter:a "volume=${volume_change}" -c:a aac -q:a 0 "$boosted_file"
    echo "Boosted audio saved to $boosted_file"
    temp_files+=("$boosted_file")
  done

  # Remux the boosted audio tracks back into the original file, preserving the original track as the last
  echo "Remuxing the boosted audio back to $temp_file..."
  mkvmerge -o "$temp_file" "${temp_files[@]}" "$source_file" --audio-tracks 0
  if [ -f "$temp_file" ]; then
    mv "$temp_file" "$output_file"
