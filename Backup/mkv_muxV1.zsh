...
boost_audio_volume() {
  local source_file="$1"
  local track_id="$2"
  local volume_change="$3"
  local codec_extension="$4"
  local extracted_file="${track_id}.${codec_extension}"
  local boosted_file="${volume_change}dB.${codec_extension}"
  local temp_file="${source_file%.*}_temp.mkv"
  local output_file="${source_file%.*}.mkv"
  local track_name="${volume_change}dB"
  ...
  echo "Boosting volume by ${volume_change}dB..."
  ffmpeg -y -i "$extracted_file" -filter:a "volume=${volume_change}dB" -c:a aac -q:a 0 "$boosted_file"
  echo "Boosted audio saved to $boosted_file"

  echo "Remuxing the boosted audio back to $output_file..."
  mkvmerge -o "$temp_file" --track-name 0:"$track_name" "$boosted_file" "$source_file" --audio-tracks 0
  if [ -f "$temp_file" ]; then
    mv "$temp_file" "$output_file"
...
extract_tracks_excluding() {
  ...
  IFS=',' read -r -A raw_ids <<< "$exclude_tracks"
...
if [ $# -gt 0 ]; then
  ...
  if [ "$choice" -eq 1 ]; then
    ...
    printf "Enter the amount of dB to change (e.g., 13, -4, 3.23): "
    read volume_change

    boost_audio_volume "$source_file" "$track_id" "$volume_change" "$codec_extension"
