...
safe_mode_write="${safe_mode_write:-false}"  # Set to false by default if not set in the environment
...
if [ $# -gt 0 ]; then
  ...
  if [ "$choice" -eq 1 ]; then
    printf "Select the source video file:\n"
    source_file=$(find . -type f | fzf --height 40% --reverse --border)
    ...
    while true; do
      ...
      source_file=$(find . -type f -name "*.mkv" | fzf --height 40% --reverse --border)
      if [ -z "$source_file" ]; then
        echo "No file selected. Exiting."
        exit 1
      fi
